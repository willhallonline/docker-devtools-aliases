...
# 16. docker_service_alias: argument validation
echo ""
echo "docker_service_alias: argument validation"
output=$(docker_service_alias 2>&1) || true
assert_contains "service-too-few-args: stderr message" "requires a container name and image" "$output"

rc=0; docker_service_alias 2>/dev/null || rc=$?
assert_exit "service-too-few-args: exit code 2" 2 "$rc"

# 17. docker_service_alias: basic argv construction (detached, named, service args)
echo ""
echo "docker_service_alias: basic argv construction"
docker_service_alias devtools-nginx nginx:latest -p 8080:80 -v /tmp:/usr/share/nginx/html:ro --
argv=$(_read_stub)
assert_line_eq "service-basic: run sub-command" 1 "run" "$argv"
assert_line_eq "service-basic: -d flag" 2 "-d" "$argv"
assert_line_eq "service-basic: --rm flag" 3 "--rm" "$argv"
assert_contains "service-basic: --name flag" "--name" "$argv"
assert_contains "service-basic: container name" "devtools-nginx" "$argv"
assert_contains "service-basic: port mapping" "8080:80" "$argv"
assert_contains "service-basic: volume mapping" "/usr/share/nginx/html:ro" "$argv"
assert_contains "service-basic: image present" "nginx:latest" "$argv"
assert_not_contains "service-basic: host user mapping disabled by default" "--user" "$argv"

# 18. docker_service_alias: command override args after "--" are forwarded
echo ""
echo "docker_service_alias: command override passthrough"
docker_service_alias devtools-postgresql postgres:latest -e POSTGRES_PASSWORD=postgres -- --extra-flag
argv=$(_read_stub)
assert_contains "service-override: env var present" "POSTGRES_PASSWORD=postgres" "$argv"
assert_contains "service-override: passthrough flag" "--extra-flag" "$argv"

# 19. docker_service_alias: DOCKER_DEVTOOLS_MAP_HOST_USER adds --user uid:gid
echo ""
echo "docker_service_alias: DOCKER_DEVTOOLS_MAP_HOST_USER"
(
    export DOCKER_DEVTOOLS_MAP_HOST_USER=true
    docker_service_alias devtools-nginx nginx:latest --
)
argv=$(_read_stub)
assert_contains "service-map-host-user: user flag present" "--user" "$argv"
assert_contains "service-map-host-user: uid gid present" "$(id -u):$(id -g)" "$argv"

# 20. docker_service_alias: DOCKER_DEVTOOLS_EXTRA_ARGS inserted before image
echo ""
echo "docker_service_alias: DOCKER_DEVTOOLS_EXTRA_ARGS"
(
    export DOCKER_DEVTOOLS_EXTRA_ARGS="--network host"
    docker_service_alias devtools-nginx nginx:latest --
)
argv=$(_read_stub)
assert_contains "service-extra-args: network flag present" "--network" "$argv"
assert_contains "service-extra-args: network value present" "host" "$argv"

# 21. docker_service_alias: docker not in PATH → exit 127
echo ""
echo "docker_service_alias: docker not in PATH"
rc=0
(
    command() {
        if [[ "${1:-}" == "-v" && "${2:-}" == "docker" ]]; then return 1; fi
    ...
    export -f command
    docker_service_alias devtools-nginx nginx:latest >/dev/null 2>&1
) || rc=$?
assert_exit "service-no-docker: exit code 127" 127 "$rc"
